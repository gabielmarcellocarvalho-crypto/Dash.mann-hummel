        ...
        <div className="overflow-x-auto">
          <table className="w-full min-w-[860px] border-collapse">
            <thead>
              <tr className="border-b border-border">
                {["Canal", "Campanha", "Status", "Cliques", "Impr.", "CTR", "CPC", "Custo", "ROAS", "ACOS", "Receita"].map(
                  (h, i) => (
                    <th
                      key={h}
              ...
              {campaigns.map((c) => {
                ...
                return (
                  <tr key={`${c.channelId}-${c.campaignId}`} className="border-b border-border/60 last:border-0">
                    ...
                    <td className="max-w-[240px] truncate py-3 text-[12.5px] font-medium text-text-1" title={c.campaign}>
                      {c.campaign}
                    </td>
                    <td className="py-3 text-right text-[12px] text-text-2">
                      {c.status ? (STATUS_LABEL[c.status] ?? c.status) : "—"}
                    </td>
                    <td className="py-3 text-right font-mono text-[12.5px] text-text-2 tabular-nums">
          ...
          {campaigns.length === 40 && (
            <p className="mt-3 text-center text-[11px] text-text-3">
              Mostrando as 40 campanhas de maior investimento no período.
            </p>
          )}
